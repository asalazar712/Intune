...
# Parameter DeviceID in Cmdlet Get-MGDevice references the AAD records Object ID, NOT THE DEVICE ID. That's why I use the filter paramter
...
# Deleting Azure AD device uses delegated permissions, unable to delete programatically 
If ($PSBoundParameters.ContainsKey("AAD") -or $PSBoundParameters.ContainsKey("All"))
{
    Try
    {
        ...
        [array]$AzureADDevices = get-mgdevice -Filter "DeviceID eq '$DeviceId'"  -ErrorAction Stop
...
If ($PSBoundParameters.ContainsKey("Intune")  -or $PSBoundParameters.ContainsKey("All"))
{
    Try
    {
        ...
        [array]$IntuneDevices = Get-MgDeviceManagementManagedDevice -Filter "AzureAdDeviceId eq '$DeviceId'" -ErrorAction Stop
